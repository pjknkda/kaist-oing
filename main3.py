...
def img_overlay(img, faces):
    ...
    face_map_overaly = np.clip(1 - face_map_overaly, 0.1, 1)

    img_fg_overlay = np.zeros((IMAGE_SIZE[1], IMAGE_SIZE[0]))
    for iter_img_fg in img_fg_list[-FG_CUMMULATE_LENGTH:]:
    ...
    overaly = face_map_overaly * img_fg_overlay + 0.2
    overaly = np.clip(overaly, 0, 1)

    overaly_heatmap = np.array(overaly * 255, dtype=np.uint8)
    overaly_heatmap = cv2.applyColorMap(overaly_heatmap, cv2.COLORMAP_JET)

    merged_img = np.array(img * overaly, dtype=np.uint8)

    cv2.imshow('Oing', merged_img)
    cv2.imshow('Oing Heatmap', overaly_heatmap)
